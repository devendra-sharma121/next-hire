...
router.post('/register', async (req, res) => {
  try {
    ...
    // Admin accounts can never be created through public self-registration.
    // Only 'recruiter' (subject to approval) or 'candidate' (default) are allowed here.
    if (role === 'recruiter') {
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Block rejected recruiters (only relevant while they're still in the recruiter role)
